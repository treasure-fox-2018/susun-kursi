// bila ada 10 orang, ['a','b','c','d','e','f','g','h','i','j'].
// orang-orang tsb akan diatur tempat duduk nya berdasarkan urutannya secara vertikal,
// dan jumlah maksimal kursi per baris vertikal adalah 3.
// tampilkan siapa saja yg berada di baris 1, 2, dan 3 nya.

// hasil yg benar :
// Baris 0 :  [ 'a', 'd', 'g', 'j' ]
// Baris 1 :  [ 'b', 'e', 'h' ]
// Baris 2 :  [ 'c', 'f', 'i' ]

'use strict'

function generateSeats(row) {
	let seats = []
	for(let i=0; i<row; i++) { // kelebihan 'a' di kondisi => i<rowa
		seats.push([]) // kurang 's' di seats
	}
	return seats
}

function managePerson(arr, rowSeats) {
	let seats = generateSeats(rowSeats) // kurang 's' di generateSeat
	let row = 0

	for(let i=0; i<arr.length-1; i++) {
		seats[row].push(arr[i])  //kurang 's' di seats[row]

		if(row < 2) { // tidak pake sama dengan
			row++
		} else {
			row = 0 // kebanyakan sama dengan
		}
	}
	printSeats(seats)
}-h

function printSeats(seats) {
	for(let i=0; i<seats.length; i++) { // var mulai dari 0
		console.log(`Baris ${i} : `, seats[i])
	}
}

managePerson(['a','b','c','d','e','f','g','h','i','j'], 3)
